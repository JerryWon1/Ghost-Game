...
public class WorldParser {
    ...
    private static Entity parseEntity(String parameters, ImageLibrary imageLibrary) {
        ...
        if (args.length >= Entity.ENTITY_PROPERTY_COLUMN_COUNT) {
            ...
            return switch (key) {
                ...
                case Ghost.GHOST_KEY -> parseGhost(specificProperties, id, position, imageLibrary);
    ...
    /** Parses a line of Ghost data. */
    private static Entity parseGhost(String[] properties, String id, Point position, ImageLibrary imageLibrary) {
        if (properties.length == Ghost.GHOST_PARSE_PROPERTY_COUNT) {
            ...
            return new Ghost(
                    id,
                    position,
                    imageLibrary.get(Ghost.GHOST_KEY),
                    Double.parseDouble(properties[Ghost.GHOST_PARSE_PROPERTY_ANIMATION_PERIOD_INDEX]),
                    Double.parseDouble(properties[Ghost.GHOST_PARSE_PROPERTY_BEHAVIOR_PERIOD_INDEX])
            );
            ...
            throw new IllegalArgumentException(String.format(
                    "%s requires %d properties when parsing, got %d",
                    Ghost.GHOST_KEY,
                    Ghost.GHOST_PARSE_PROPERTY_COUNT,
                    properties.length
            ));
        }
    }
